...
public class DashboardController {
    ...
    private void loadFromDatabase() {
        try {
            ...
            // Total orders (standard ReactiveCrudRepository.count())
            Long total = orderRepository.count().block();
            if (total != null) {
                totalOrders.set(total.intValue());
            }

            // Today's orders:
            // kept purely in-memory, updated by recordRevenue()
            // and reset daily by resetDailyCountersIfNeeded().
            // No DB-backed "today-only" query available here.
