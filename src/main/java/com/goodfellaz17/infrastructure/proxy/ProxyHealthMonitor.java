...
public class ProxyHealthMonitor {
    ...
    /**
     * Cleanup: Shutdown executor service when bean is destroyed.
     */
    @PreDestroy
    public void shutdownExecutor() {
        log.info("🛑 Shutting down proxy health monitor executor...");
        if (executor != null && !executor.isShutdown()) {
            executor.shutdown();
            try {
                if (!executor.awaitTermination(10, TimeUnit.SECONDS)) {
                    log.warn("⚠️ Force shutdown executor after timeout");
