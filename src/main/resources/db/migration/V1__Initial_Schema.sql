...
-- 
-- Database: PostgreSQL (pure SQL, no vendor-specific keywords)
-- Application: goodfellaz17 - Spotify Bot Delivery Engine
-- 
-- Tables:
--   - users: Platform users with tiered pricing
--   - services: Available Spotify engagement services
--   - orders: Delivery orders (aggregate root)
--   - balance_transactions: Financial ledger
--   - proxy_nodes: Distributed proxy pool
--   - proxy_metrics: Real-time proxy health metrics
--   - device_nodes: Emulator/device farm
--   - tor_circuits: Tor exit node circuits
-- 
...
COMMENT ON TABLE users IS 'Platform users with tiered pricing (CONSUMER/RESELLER/AGENCY)';
COMMENT ON COLUMN users.tier IS 'Pricing tier: CONSUMER (base), RESELLER (20-30% off), AGENCY (40-50% off)';
COMMENT ON COLUMN users.api_key IS '64-char key for programmatic API access';
...
COMMENT ON TABLE services IS 'Catalog of Spotify engagement services';
COMMENT ON COLUMN services.geo_profiles IS 'JSON array of supported GeoProfile values';
...
CREATE TABLE orders (
    ...
    CONSTRAINT orders_target_url_valid CHECK (target_url ~ '^https://(open\.)?spotify\.com/.*$')
);
...
COMMENT ON TABLE orders IS 'Spotify engagement delivery orders (aggregate root)';
COMMENT ON COLUMN orders.start_count IS 'Initial metric count when order started';
COMMENT ON COLUMN orders.current_count IS 'Latest scraped metric count';
COMMENT ON COLUMN orders.external_order_id IS 'ID from external API integrations';
...
CREATE TABLE balance_transactions (
    ...
    CONSTRAINT balance_tx_balance_consistent CHECK (
        (type = 'DEBIT' AND amount < 0) OR
        (type IN ('CREDIT', 'REFUND', 'BONUS') AND amount > 0) OR
        (type = 'ADJUSTMENT')
    )
);
...
CREATE INDEX idx_balance_tx_external ON balance_transactions(external_tx_id) WHERE external_tx_id IS NOT NULL;

COMMENT ON TABLE balance_transactions IS 'Financial ledger for user balance changes';
COMMENT ON COLUMN balance_transactions.amount IS 'Positive for credits, negative for debits';
...
CREATE INDEX idx_proxy_nodes_available ON proxy_nodes(tier, status, current_load) 
    WHERE status = 'ONLINE';

COMMENT ON TABLE proxy_nodes IS 'Distributed proxy pool across multiple providers';
COMMENT ON COLUMN proxy_nodes.tier IS 'Quality tier: DATACENTER < ISP < TOR < RESIDENTIAL < MOBILE';
COMMENT ON COLUMN proxy_nodes.tags IS 'JSON array for flexible categorization';
...
CREATE TABLE proxy_metrics (
    ...
    CONSTRAINT proxy_metrics_latency_positive CHECK (
        latency_p50 >= 0 AND latency_p95 >= 0 AND latency_p99 >= 0
    )
);
...
COMMENT ON TABLE proxy_metrics IS 'Real-time rolling metrics for proxy health monitoring';
COMMENT ON COLUMN proxy_metrics.error_codes IS 'JSON object: {"401": count, "403": count, "429": count}';
COMMENT ON COLUMN proxy_metrics.window_start IS 'Start of current rolling metrics window';
...
CREATE INDEX idx_device_nodes_available ON device_nodes(device_type, status, active_sessions) 
    WHERE status = 'ACTIVE';

COMMENT ON TABLE device_nodes IS 'Emulator/device farm for Spotify client simulation';
COMMENT ON COLUMN device_nodes.fingerprint IS 'Device fingerprint data for anti-detection';
...
CREATE INDEX idx_tor_circuits_active ON tor_circuits(status, success_rate DESC) 
    WHERE status = 'ACTIVE';

COMMENT ON TABLE tor_circuits IS 'Tor exit node circuits for anonymized traffic';
COMMENT ON COLUMN tor_circuits.exit_geo IS 'ISO 3166-1 alpha-2 country code of exit node';
...
-- =============================================================================
-- INITIAL DATA (Services Catalog)
-- =============================================================================
INSERT INTO services (name, display_name, service_type, description, 
                      cost_per_1k, reseller_cost_per_1k, agency_cost_per_1k,
                      min_quantity, max_quantity, estimated_days_min, estimated_days_max,
                      geo_profiles, sort_order)
VALUES 
    ('plays_ww', 'Worldwide Plays', 'PLAYS', 
     'Global stream distribution from mixed geo sources', 
     2.50, 2.00, 1.50, 1000, 10000000, 1, 7, 
     '["WORLDWIDE"]', 1),
    
    ('plays_usa', 'USA Premium Plays', 'PLAYS', 
     'High-quality streams from verified US accounts', 
     4.50, 3.60, 2.70, 1000, 5000000, 2, 10, 
     '["USA"]', 2),
    
    ('plays_eu', 'European Plays', 'PLAYS', 
     'Streams from EU region (UK, DE, FR, etc.)', 
     3.50, 2.80, 2.10, 1000, 5000000, 2, 10, 
     '["EUROPE", "UK", "DE", "FR"]', 3),
    
    ('monthly_listeners', 'Monthly Listeners', 'MONTHLY_LISTENERS', 
     'Unique monthly listeners (28-day retention)', 
     8.00, 6.40, 4.80, 100, 100000, 7, 30, 
     '["WORLDWIDE", "USA", "EUROPE"]', 4),
    
    ('saves', 'Track Saves', 'SAVES', 
     'Save tracks to user libraries', 
     5.00, 4.00, 3.00, 100, 500000, 1, 7, 
     '["WORLDWIDE"]', 5),
    
    ('follows', 'Artist Follows', 'FOLLOWS', 
     'Followers for artist profiles', 
     6.00, 4.80, 3.60, 100, 500000, 1, 7, 
     '["WORLDWIDE"]', 6),
    
    ('playlist_follows', 'Playlist Followers', 'PLAYLIST_FOLLOWERS', 
     'Followers for public playlists', 
     4.00, 3.20, 2.40, 100, 100000, 1, 5, 
     '["WORLDWIDE"]', 7);

-- =============================================================================
-- GRANTS (Adjust for your database user)
-- =============================================================================
-- GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO goodfellaz17_app;
-- GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO goodfellaz17_app;
